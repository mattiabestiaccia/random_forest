...
# RGB Random Forest - Poisson Experiments Runner
# ==============================================
# This script runs experiments on poisson datasets in sequence:
# 1. dataset_rgb_poisson_40
# 2. dataset_rgb_poisson_60  
# 3. dataset_rgb_poisson_100
# All output folders will be created in: ./poisson_output/
# 
# Parameters:
...
#
# Usage: ./run_poisson_experiments.sh [base_dataset_dir]
#        ./run_poisson_experiments.sh /custom/path/to/dataset_rgb

# Configuration
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PYTHON_SCRIPT="/home/brusc/Projects/random_forest/scripts/train_and_save_model.py"
OUTPUT_BASE_DIR="$SCRIPT_DIR/poisson_output"
...
# Poisson experiment configurations - processing in sequence by intensity
POISSON_DATASETS=("dataset_rgb_poisson_40" "dataset_rgb_poisson_60" "dataset_rgb_poisson_100")
...
is_experiment_completed() {
    local poisson_dataset=$1
    local dataset_type=$2
    local area=$3
    local feature_method=$4
    local k_features=$5
    
    # Create the structured output directory path: poisson_dataset/area/dataset_type/k{features}/feature_method/
    local output_dir="$OUTPUT_BASE_DIR/$poisson_dataset/$area/$dataset_type/k${k_features}/$feature_method"
...
run_experiment() {
    ...
    local experiment_name="${poisson_dataset}_${dataset_type}_${area}_${feature_method}_k${k_features}"
    ...
    if is_experiment_completed "$poisson_dataset" "$dataset_type" "$area" "$feature_method" "$k_features"; then
    ...
    # Create the structured output directory path: poisson_dataset/area/dataset_type/k{features}/feature_method/
    local structured_output_dir="$OUTPUT_BASE_DIR/$poisson_dataset/$area/$dataset_type/k${k_features}/$feature_method"
    
    # Create the output directory structure
    mkdir -p "$structured_output_dir"
    
    # Determine the dataset path
    local dataset_path="$BASE_DATASET_DIR/$poisson_dataset/$dataset_type"
...
generate_summary() {
    local summary_file="$RESULTS_DIR/poisson_experiment_summary.txt"
    local csv_file="$RESULTS_DIR/poisson_experiment_results.csv"
    
    log_message "INFO" "Generating summary report..."
    
    # Create CSV header
    echo "PoissonDataset,Dataset,Area,FeatureMethod,K,Accuracy,StdDev,Status" > "$csv_file"
    
    # Create summary header
    {
        echo "RGB Random Forest - Poisson Experiments Summary"
        echo "==============================================="
        echo "Generated: $(date)"
        echo "Base Dataset Directory: $BASE_DATASET_DIR"
        echo "Output Directory: $OUTPUT_BASE_DIR"
        echo ""
        echo "Processed Datasets: ${POISSON_DATASETS[*]}"
        echo ""
    ...
    for poisson_dataset in "${POISSON_DATASETS[@]}"; do
        echo "" >> "$summary_file"
        echo "=== $poisson_dataset ===" >> "$summary_file"
                    ...
                    for k_features in "${K_VALUES[@]}"; do
                        ...
                        if [ -f "$log_file" ] && [ ! -s "$error_file" ]; then
                            ...
                            if [ -n "$accuracy" ] && [ -n "$std_dev" ]; then
                                ...
                                # Add to CSV
                                echo "$poisson_dataset,$dataset_type,$area,$feature_method,$k_features,$accuracy,$std_dev,SUCCESS" >> "$csv_file"
                                
                                # Add to summary
                                echo "✓ $experiment_name: ${accuracy} ± ${std_dev}" >> "$summary_file"
                            else
                                echo "$poisson_dataset,$dataset_type,$area,$feature_method,$k_features,,,FAILED" >> "$csv_file"
    ...
    # Add summary statistics
    {
        ...
        for poisson_dataset in "${POISSON_DATASETS[@]}"; do
            local dataset_experiments=$(grep "^$poisson_dataset," "$csv_file" | wc -l)
            local dataset_success=$(grep "^$poisson_dataset,.*SUCCESS$" "$csv_file" | wc -l)
            echo "  $poisson_dataset: $dataset_success/$dataset_experiments successful"
        done
    } >> "$summary_file"
...
# Main execution
main() {
    log_message "INFO" "Starting RGB Random Forest - Poisson Experiments"
    ...
    log_message "INFO" "Processing datasets in sequence: ${POISSON_DATASETS[*]}"
    
    # Calculate total experiments
    local total_experiments=$(( ${#POISSON_DATASETS[@]} * ${#DATASET_TYPES[@]} * ${#AREAS[@]} * ${#FEATURE_METHODS[@]} * ${#K_VALUES[@]} ))
    ...
    # Run all experiments - process poisson datasets in sequence
    for poisson_dataset in "${POISSON_DATASETS[@]}"; do
        log_message "INFO" "Starting poisson dataset: $poisson_dataset"
        for dataset_type in "${DATASET_TYPES[@]}"; do
                ...
                for feature_method in "${FEATURE_METHODS[@]}"; do
                    ...
                    for k_features in "${K_VALUES[@]}"; do
                        ...
                        if is_experiment_completed "$poisson_dataset" "$dataset_type" "$area" "$feature_method" "$k_features"; then
                            ...
                            if run_experiment "$poisson_dataset" "$dataset_type" "$area" "$feature_method" "$k_features"; then
                                successful_experiments=$((successful_experiments + 1))
        ...
        log_message "INFO" "Completed poisson dataset: $poisson_dataset"
        echo ""
    done
    
    # Generate summary
    echo ""
    log_message "INFO" "All poisson experiments completed!"
    ...
    # Final message
    echo ""
    log_message "INFO" "All poisson experiments finished!"
...
trap 'log_message "WARNING" "Poisson experiments interrupted by user"; exit 1' INT
