...
# RGB Random Forest - Gaussian Experiments Runner
# ==============================================
# This script runs experiments on gaussian datasets in sequence:
# 1. rgb_clean_kbest
# 2. rgb_gaussian30_kbest  
# 3. rgb_gaussian50_kbest
# All output folders will be created in: ./gaussian_output/
# 
# Parameters:
# - dataset_type: mini, small, original
# - area_name: assatigue, popolar, sunset
# - feature_method: wst, advanced_stats, hybrid
# - k_features: 2, 5, 10, 20
#
# Usage: ./run_gaussian_experiments.sh [base_dataset_dir]
#        ./run_gaussian_experiments.sh /custom/path/to/dataset_rgb

# Configuration
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PYTHON_SCRIPT="/home/brusc/Projects/random_forest/scripts/train_and_save_model.py"
OUTPUT_BASE_DIR="$SCRIPT_DIR/gaussian_output"
...
# Gaussian experiment configurations - processing in sequence
GAUSSIAN_DATASETS=("rgb_clean" "rgb_gaussian30" "rgb_gaussian50")
...
is_experiment_completed() {
    local gaussian_dataset=$1
    local dataset_type=$2
    local area=$3
    local feature_method=$4
    local k_features=$5
    
    # Create the structured output directory path: gaussian_dataset/area/dataset_type/k{features}/feature_method/
    local output_dir="$OUTPUT_BASE_DIR/$gaussian_dataset/$area/$dataset_type/k${k_features}/$feature_method"
...
run_experiment() {
    ...
    local experiment_name="${gaussian_dataset}_${dataset_type}_${area}_${feature_method}_k${k_features}"
    ...
    if is_experiment_completed "$gaussian_dataset" "$dataset_type" "$area" "$feature_method" "$k_features"; then
    ...
    # Create the structured output directory path: gaussian_dataset/area/dataset_type/k{features}/feature_method/
    local structured_output_dir="$OUTPUT_BASE_DIR/$gaussian_dataset/$area/$dataset_type/k${k_features}/$feature_method"
    
    # Create the output directory structure
    mkdir -p "$structured_output_dir"
    
    # Determine the dataset path based on gaussian_dataset type
    if [ "$gaussian_dataset" == "rgb_clean" ]; then
        dataset_path="$BASE_DATASET_DIR/dataset_rgb_clean/$dataset_type"
    elif [ "$gaussian_dataset" == "rgb_gaussian30" ]; then
        dataset_path="$BASE_DATASET_DIR/dataset_rgb_gaussian_30/$dataset_type"
    elif [ "$gaussian_dataset" == "rgb_gaussian50" ]; then
        dataset_path="$BASE_DATASET_DIR/dataset_rgb_gaussian_50/$dataset_type"
    fi
    ...
    if (cd "$OUTPUT_BASE_DIR" && python3 "$PYTHON_SCRIPT" "$dataset_path" "$area" "$feature_method" "$k_features" "$structured_output_dir") > "$log_file" 2> "$error_file"; then
...
generate_summary() {
    local summary_file="$RESULTS_DIR/gaussian_experiment_summary.txt"
    local csv_file="$RESULTS_DIR/gaussian_experiment_results.csv"
    
    log_message "INFO" "Generating summary report..."
    
    # Create CSV header
    echo "GaussianDataset,Dataset,Area,FeatureMethod,K,Accuracy,StdDev,Status" > "$csv_file"
    
    # Create summary header
    {
        echo "RGB Random Forest - Gaussian Experiments Summary"
        echo "================================================"
        echo "Generated: $(date)"
        echo "Base Dataset Directory: $BASE_DATASET_DIR"
        echo "Output Directory: $OUTPUT_BASE_DIR"
        echo ""
        echo "Processed Datasets: ${GAUSSIAN_DATASETS[*]}"
        echo ""
    ...
    for gaussian_dataset in "${GAUSSIAN_DATASETS[@]}"; do
        echo "" >> "$summary_file"
        echo "=== $gaussian_dataset ===" >> "$summary_file"
                    ...
                    for k_features in "${K_VALUES[@]}"; do
                        ...
                        if [ -f "$log_file" ] && [ ! -s "$error_file" ]; then
                            ...
                            if [ -n "$accuracy" ] && [ -n "$std_dev" ]; then
                                ...
                                # Add to CSV
                                echo "$gaussian_dataset,$dataset_type,$area,$feature_method,$k_features,$accuracy,$std_dev,SUCCESS" >> "$csv_file"
                                
                                # Add to summary
                                echo "✓ $experiment_name: ${accuracy} ± ${std_dev}" >> "$summary_file"
                            else
                                echo "$gaussian_dataset,$dataset_type,$area,$feature_method,$k_features,,,FAILED" >> "$csv_file"
    ...
    # Add summary statistics
    {
        ...
        for gaussian_dataset in "${GAUSSIAN_DATASETS[@]}"; do
            local dataset_experiments=$(grep "^$gaussian_dataset," "$csv_file" | wc -l)
            local dataset_success=$(grep "^$gaussian_dataset,.*SUCCESS$" "$csv_file" | wc -l)
            echo "  $gaussian_dataset: $dataset_success/$dataset_experiments successful"
        done
    } >> "$summary_file"
...
# Main execution
main() {
    log_message "INFO" "Starting RGB Random Forest - Gaussian Experiments"
    ...
    log_message "INFO" "Processing datasets in sequence: ${GAUSSIAN_DATASETS[*]}"
    
    # Calculate total experiments
    local total_experiments=$(( ${#GAUSSIAN_DATASETS[@]} * ${#DATASET_TYPES[@]} * ${#AREAS[@]} * ${#FEATURE_METHODS[@]} * ${#K_VALUES[@]} ))
    ...
    # Run all experiments - process gaussian datasets in sequence
    for gaussian_dataset in "${GAUSSIAN_DATASETS[@]}"; do
        log_message "INFO" "Starting gaussian dataset: $gaussian_dataset"
        for dataset_type in "${DATASET_TYPES[@]}"; do
                ...
                for feature_method in "${FEATURE_METHODS[@]}"; do
                    ...
                    for k_features in "${K_VALUES[@]}"; do
                        ...
                        if is_experiment_completed "$gaussian_dataset" "$dataset_type" "$area" "$feature_method" "$k_features"; then
                            ...
                            if run_experiment "$gaussian_dataset" "$dataset_type" "$area" "$feature_method" "$k_features"; then
                                successful_experiments=$((successful_experiments + 1))
        ...
        log_message "INFO" "Completed gaussian dataset: $gaussian_dataset"
        echo ""
    done
    
    # Generate summary
    echo ""
    log_message "INFO" "All gaussian experiments completed!"
    ...
    # Final message
    echo ""
    log_message "INFO" "All gaussian experiments finished!"
...
trap 'log_message "WARNING" "Gaussian experiments interrupted by user"; exit 1' INT
