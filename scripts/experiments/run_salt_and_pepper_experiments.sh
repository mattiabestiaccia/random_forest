...
# RGB Random Forest - Salt and Pepper Experiments Runner
# ======================================================
# This script runs experiments on salt and pepper datasets in sequence:
# 1. dataset_rgb_salt_and_pepper_5
# 2. dataset_rgb_salt_and_pepper_15  
# 3. dataset_rgb_salt_and_pepper_25
# All output folders will be created in: ./salt_and_pepper_output/
# 
# Parameters:
...
#
# Usage: ./run_salt_and_pepper_experiments.sh [base_dataset_dir]

# Configuration
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PYTHON_SCRIPT="/home/brusc/Projects/random_forest/scripts/train_and_save_model.py"
OUTPUT_BASE_DIR="$SCRIPT_DIR/salt_and_pepper_output"
...
# Salt and Pepper experiment configurations - processing in sequence by intensity
SALT_PEPPER_DATASETS=("dataset_rgb_salt_and_pepper_5" "dataset_rgb_salt_and_pepper_15" "dataset_rgb_salt_and_pepper_25")
...
is_experiment_completed() {
    local salt_pepper_dataset=$1
    local dataset_type=$2
    local area=$3
    local feature_method=$4
    local k_features=$5
    
    # Create the structured output directory path: salt_pepper_dataset/area/dataset_type/k{features}/feature_method/
    local output_dir="$OUTPUT_BASE_DIR/$salt_pepper_dataset/$area/$dataset_type/k${k_features}/$feature_method"
...
run_experiment() {
    ...
    local experiment_name="${salt_pepper_dataset}_${dataset_type}_${area}_${feature_method}_k${k_features}"
    ...
    if is_experiment_completed "$salt_pepper_dataset" "$dataset_type" "$area" "$feature_method" "$k_features"; then
    ...
    # Create the structured output directory path: salt_pepper_dataset/area/dataset_type/k{features}/feature_method/
    local structured_output_dir="$OUTPUT_BASE_DIR/$salt_pepper_dataset/$area/$dataset_type/k${k_features}/$feature_method"
    
    # Create the output directory structure
    mkdir -p "$structured_output_dir"
    
    # Determine the dataset path
    local dataset_path="$BASE_DATASET_DIR/$salt_pepper_dataset/$dataset_type"
...
generate_summary() {
    local summary_file="$RESULTS_DIR/salt_and_pepper_experiment_summary.txt"
    local csv_file="$RESULTS_DIR/salt_and_pepper_experiment_results.csv"
    
    log_message "INFO" "Generating summary report..."
    
    # Create CSV header
    echo "SaltPepperDataset,Dataset,Area,FeatureMethod,K,Accuracy,StdDev,Status" > "$csv_file"
    
    # Create summary header
    {
        echo "RGB Random Forest - Salt and Pepper Experiments Summary"
        echo "======================================================="
        echo "Generated: $(date)"
        echo "Base Dataset Directory: $BASE_DATASET_DIR"
        echo "Output Directory: $OUTPUT_BASE_DIR"
        echo ""
        echo "Processed Datasets: ${SALT_PEPPER_DATASETS[*]}"
        echo ""
    ...
    for salt_pepper_dataset in "${SALT_PEPPER_DATASETS[@]}"; do
        echo "" >> "$summary_file"
        echo "=== $salt_pepper_dataset ===" >> "$summary_file"
                    ...
                    for k_features in "${K_VALUES[@]}"; do
                        ...
                        if [ -f "$log_file" ] && [ ! -s "$error_file" ]; then
                            ...
                            if [ -n "$accuracy" ] && [ -n "$std_dev" ]; then
                                ...
                                # Add to CSV
                                echo "$salt_pepper_dataset,$dataset_type,$area,$feature_method,$k_features,$accuracy,$std_dev,SUCCESS" >> "$csv_file"
                                
                                # Add to summary
                                echo "✓ $experiment_name: ${accuracy} ± ${std_dev}" >> "$summary_file"
                            else
                                echo "$salt_pepper_dataset,$dataset_type,$area,$feature_method,$k_features,,,FAILED" >> "$csv_file"
    ...
    # Add summary statistics
    {
        ...
        for salt_pepper_dataset in "${SALT_PEPPER_DATASETS[@]}"; do
            local dataset_experiments=$(grep "^$salt_pepper_dataset," "$csv_file" | wc -l)
            local dataset_success=$(grep "^$salt_pepper_dataset,.*SUCCESS$" "$csv_file" | wc -l)
            echo "  $salt_pepper_dataset: $dataset_success/$dataset_experiments successful"
        done
    } >> "$summary_file"
...
# Main execution
main() {
    log_message "INFO" "Starting RGB Random Forest - Salt and Pepper Experiments"
    ...
    log_message "INFO" "Processing datasets in sequence: ${SALT_PEPPER_DATASETS[*]}"
    
    # Calculate total experiments
    local total_experiments=$(( ${#SALT_PEPPER_DATASETS[@]} * ${#DATASET_TYPES[@]} * ${#AREAS[@]} * ${#FEATURE_METHODS[@]} * ${#K_VALUES[@]} ))
    ...
    # Run all experiments - process salt and pepper datasets in sequence
    for salt_pepper_dataset in "${SALT_PEPPER_DATASETS[@]}"; do
        log_message "INFO" "Starting salt and pepper dataset: $salt_pepper_dataset"
        for dataset_type in "${DATASET_TYPES[@]}"; do
                ...
                for feature_method in "${FEATURE_METHODS[@]}"; do
                    ...
                    for k_features in "${K_VALUES[@]}"; do
                        ...
                        if is_experiment_completed "$salt_pepper_dataset" "$dataset_type" "$area" "$feature_method" "$k_features"; then
                            ...
                            if run_experiment "$salt_pepper_dataset" "$dataset_type" "$area" "$feature_method" "$k_features"; then
                                successful_experiments=$((successful_experiments + 1))
        ...
        log_message "INFO" "Completed salt and pepper dataset: $salt_pepper_dataset"
        echo ""
    done
    
    # Generate summary
    echo ""
    log_message "INFO" "All salt and pepper experiments completed!"
    ...
    # Final message
    echo ""
    log_message "INFO" "All salt and pepper experiments finished!"
...
trap 'log_message "WARNING" "Salt and pepper experiments interrupted by user"; exit 1' INT
