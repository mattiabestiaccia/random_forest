#!/usr/bin/env python3
"""
Complete analysis script for poisson noise experiments.
...
class PoissonExperimentAnalyzer:
    def __init__(self, base_dir: str = "/home/brusc/Projects/random_forest/experiments_organized/poisson_output"):
        self.base_dir = Path(base_dir)
        self.results = []
        self.noise_conditions = ['clean', 'poisson40', 'poisson60']
        ...
        self.noise_labels = {
            'clean': 'Clean', 
            'poisson40': 'Poisson λ=40', 
            'poisson60': 'Poisson λ=60'
        }
    ...
    def load_all_experiments(self) -> List[Dict]:
        """Load all experiment files from poisson_output directories."""
        experiments = []
        
        # Look for directories matching the poisson pattern
        for noise_level in ['40', '60']:
            noise_dir = self.base_dir / f"dataset_rgb_poisson_{noise_level}"
                        ...
                        for method in self.feature_methods:
                            ...
                            for json_file in json_files:
                                try:
                                    ...
                                    exp_data = {
                                        'noise_condition': f'poisson{noise_level}',
                                        'area': area,
    ...
    def generate_comprehensive_report(self) -> str:
        """Generate a comprehensive summary report for poisson noise experiments."""
        if not self.results:
        ...
        report = []
        report.append("# COMPARATIVE REPORT: RANDOM FOREST EXPERIMENTS - POISSON NOISE")
        report.append("=" * 70)
        report.append("")
        ...
        report.append("under different poisson noise conditions and dataset sizes.")
        report.append("")
        ...
        report.append("## POISSON NOISE ROBUSTNESS ANALYSIS")
        ...
        for exp in self.results:
            ...
            elif exp['noise_condition'] == 'poisson40':
                poisson40_perfs[key] = exp['mean_accuracy']
            elif exp['noise_condition'] == 'poisson60':
                poisson60_perfs[key] = exp['mean_accuracy']
        
        # Calculate degradation by method
        method_degradation_40 = {}
        method_degradation_60 = {}
        
        for key in clean_perfs:
            area, dataset, method, k = key
            if method not in method_degradation_40:
                method_degradation_40[method] = []
                method_degradation_60[method] = []
            
            if key in poisson40_perfs:
                degradation = clean_perfs[key] - poisson40_perfs[key]
                method_degradation_40[method].append(degradation)
            
            if key in poisson60_perfs:
                degradation = clean_perfs[key] - poisson60_perfs[key]
                method_degradation_60[method].append(degradation)
        
        report.append("### Average degradation per method (Clean → Poisson λ=40)")
        ...
        report.append("")
        report.append("### Average degradation per method (Clean → Poisson λ=60)")
        for method in sorted(method_degradation_60.keys()):
            if method_degradation_60[method]:
                mean_deg = statistics.mean(method_degradation_60[method])
                std_deg = statistics.stdev(method_degradation_60[method]) if len(method_degradation_60[method]) > 1 else 0
                method_name = self.method_labels.get(method, method)
                report.append(f"- {method_name}: {mean_deg:.3f} ± {std_deg:.3f}")
    ...
    def generate_qualitative_analysis(self) -> str:
        """Generate qualitative analysis for poisson noise robustness."""
        if not self.results:
            self.load_all_experiments()
        ...
        analysis = []
        analysis.append("# QUALITATIVE ANALYSIS: WST vs RGB STATISTICS ROBUSTNESS TO POISSON NOISE")
        analysis.append("")
        analysis.append("## EXECUTIVE SUMMARY")
        analysis.append("")
        analysis.append("Comparative analysis reveals significant differences in poisson noise ")
        ...
        analysis.append("superior performance compared to **Advanced RGB Statistics** under poisson noise conditions.")
        analysis.append("")
        ...
        analysis.append("### 2. POISSON NOISE ROBUSTNESS")
        analysis.append("")
        ...
        analysis.append("")
        analysis.append("### 🔍 **Poisson Noise Robustness**")
        analysis.append("")
        analysis.append("1. **WST shows superior robustness**: Maintains consistent performance ")
        analysis.append("   across all poisson noise levels, particularly in SUNSET area.")
        analysis.append("")
        ...
        analysis.append("   poisson noise, especially at higher noise levels (λ=60).")
        analysis.append("")
        
        analysis.append("### 🎯 **Poisson Noise Mechanisms**")
        analysis.append("")
        analysis.append("**WST**: Multi-scale coefficients capture structural patterns that are ")
        analysis.append("less affected by shot noise characteristics of Poisson noise. The wavelet ")
        ...
        analysis.append("the signal-dependent nature of Poisson noise, which affects pixel ")
        analysis.append("intensity distributions proportionally to signal strength.")
        analysis.append("")
        
        analysis.append("### 📊 **Practical Recommendations**")
        analysis.append("")
        analysis.append("1. **For low-light/shot noise applications**: Use **WST**, especially in ")
        analysis.append("   environments with Poisson noise λ > 40.")
        analysis.append("")
    ...
    def create_comparison_plots(self, output_dir: str):
        """Create comparison plots for poisson noise experiments."""
        if not self.results:
            self.load_all_experiments()
        ...
        noise_order = ['clean', 'poisson40', 'poisson60']
        ...
        plt.ylim(0.5, 1.05)
        plt.tight_layout()
        plt.savefig(comp_dir / "accuracy_vs_poisson_noise_overall.png", dpi=300, bbox_inches='tight')
        plt.close()
        ...
        plt.title('Accuracy vs Dataset Size\n(Averaged over geographic areas, poisson noise conditions, and k values)', fontsize=14)
        ...
        plt.title('Accuracy Distribution by Method - Poisson Noise Experiments\n(All experiments)', fontsize=14)
        plt.xlabel('Feature Extraction Method', fontsize=12)
        plt.ylabel('Accuracy', fontsize=12)
        plt.grid(True, alpha=0.3)
        plt.ylim(0.5, 1.05)
        plt.tight_layout()
        plt.savefig(comp_dir / "accuracy_vs_method_boxplot_poisson.png", dpi=300, bbox_inches='tight')
        plt.close()
        ...
        plt.title('Accuracy Heatmap: Methods vs Poisson Noise-Dataset Combinations\n(Averaged over geographic areas and k values)', fontsize=12)
        plt.xlabel('Condition (Noise + Dataset)', fontsize=11)
        plt.ylabel('Feature Extraction Method', fontsize=11)
        plt.xticks(rotation=45)
        plt.tight_layout()
        plt.savefig(comp_dir / "accuracy_heatmap_poisson_summary.png", dpi=300, bbox_inches='tight')
        plt.close()
        
        print(f"✅ Poisson noise comparison plots saved to: {comp_dir}/")
    ...
    def create_detailed_plots(self, output_dir: str):
        ...
        print("Creating accuracy vs poisson noise plots...")
        for dataset_type in dataset_order:
            for k_val in k_order:
                ...
                if len(subset_data) > 0:
                    ...
                    plt.title(f'Accuracy vs Poisson Noise - {dataset_label} Dataset, k={k_val}\n(Averaged over geographic areas only)', fontsize=12)
                    ...
                    plt.ylim(0.5, 1.05)
                    plt.tight_layout()
                    plt.savefig(detail_dir / f"accuracy_vs_poisson_{dataset_type}_k{k_val}.png", dpi=300, bbox_inches='tight')
                    plot_count += 1
                plt.close()
        ...
        print(f"✅ Detailed poisson noise plots saved to: {detail_dir}/")
    ...
    def export_to_csv(self, output_dir: str):
        ...
        csv_path = Path(output_dir) / "poisson_experiments_summary.csv"
        export_df.to_csv(csv_path, index=False)
        print(f"📊 Poisson noise results exported to: {csv_path}")
    
    def create_complete_analysis(self, output_dir: str = "poisson_analysis"):
        """Create complete analysis with all plots and reports for poisson noise experiments."""
        output_path = Path(output_dir)
        output_path.mkdir(exist_ok=True)
        
        print("🚀 Starting complete poisson noise analysis...")
        ...
        with open(output_path / "poisson_comprehensive_report.md", "w", encoding="utf-8") as f:
            f.write(comprehensive_report)
        
        print("📝 Generating qualitative analysis...")
        qualitative_analysis = self.generate_qualitative_analysis()
        with open(output_path / "poisson_qualitative_analysis.md", "w", encoding="utf-8") as f:
        ...
        print(f"\n✅ Complete poisson noise analysis finished!")
    ...
    def create_analysis_summary(self, output_dir: Path):
        ...
        summary = []
        summary.append("# POISSON NOISE ANALYSIS SUMMARY")
        summary.append("=" * 50)
        summary.append("")
        ...
        summary.append("- `poisson_comprehensive_report.md`: Complete statistical analysis")
        summary.append("- `poisson_qualitative_analysis.md`: In-depth qualitative interpretation")
        summary.append("- `poisson_experiments_summary.csv`: Raw data export")
        summary.append("")
        ...
        if not df.empty:
            ...
            # Noise impact
            if 'clean' in df['noise_condition'].values and 'poisson60' in df['noise_condition'].values:
                clean_acc = df[df['noise_condition'] == 'clean']['mean_accuracy'].mean()
                poisson60_acc = df[df['noise_condition'] == 'poisson60']['mean_accuracy'].mean()
                noise_impact = (clean_acc - poisson60_acc) * 100
                summary.append(f"- **Poisson noise impact**: {noise_impact:.1f}% performance loss (Clean → Poisson λ=60)")
        
        summary.append("")
        summary.append("## METHODOLOGY")
        summary.append("")
        summary.append("- **Geographic areas**: Results averaged over assatigue, popolar, sunset")
        summary.append("- **Noise conditions**: clean, poisson40 (λ=40), poisson60 (λ=60)")
        ...
        with open(output_dir / "poisson_analysis_summary.md", "w", encoding="utf-8") as f:
            f.write("\n".join(summary))
        
        print(f"📋 Poisson analysis summary saved to: {output_dir / 'poisson_analysis_summary.md'}")

def main():
    """Main function to run complete poisson noise analysis."""
    analyzer = PoissonExperimentAnalyzer()
