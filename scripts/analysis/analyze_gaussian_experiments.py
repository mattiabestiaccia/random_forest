#!/usr/bin/env python3
"""
Complete analysis script for gaussian noise experiments.
This script generates all plots and reports comparing feature extraction methods
under different gaussian noise conditions and dataset sizes.
"""
...
class GaussianExperimentAnalyzer:
    def __init__(self, base_dir: str = "/home/brusc/Projects/random_forest/experiments"):
        ...
        self.noise_labels = {
            'clean': 'Clean', 
            'gaussian30': 'Gaussian σ=30', 
            'gaussian50': 'Gaussian σ=50'
        }
    ...
    def load_all_experiments(self) -> List[Dict]:
        """Load all experiment files from gaussian_output directories."""
        experiments = []
        
        # Look for directories matching the gaussian pattern (clean, gaussian30, gaussian50)
        for noise_condition in ['clean', 'gaussian30', 'gaussian50']:
            noise_dir = self.base_dir / f"rgb_{noise_condition}"
    ...
    def generate_comprehensive_report(self) -> str:
        """Generate a comprehensive summary report for gaussian noise experiments."""
        if not self.results:
        ...
        report = []
        report.append("# COMPARATIVE REPORT: RANDOM FOREST EXPERIMENTS - GAUSSIAN NOISE")
        report.append("=" * 70)
        report.append("")
        ...
        report.append("under different gaussian noise conditions and dataset sizes.")
        report.append("")
        ...
        report.append("## GAUSSIAN NOISE ROBUSTNESS ANALYSIS")
    ...
    def generate_qualitative_analysis(self) -> str:
        """Generate qualitative analysis for gaussian noise robustness."""
        if not self.results:
            self.load_all_experiments()
        ...
        analysis = []
        analysis.append("# QUALITATIVE ANALYSIS: WST vs RGB STATISTICS ROBUSTNESS TO GAUSSIAN NOISE")
        analysis.append("")
        analysis.append("## EXECUTIVE SUMMARY")
        analysis.append("")
        analysis.append("Comparative analysis reveals significant differences in gaussian noise ")
        ...
        analysis.append("superior performance compared to **Advanced RGB Statistics** under gaussian noise conditions.")
        analysis.append("")
        ...
        analysis.append("### 2. GAUSSIAN NOISE ROBUSTNESS")
        analysis.append("")
        ...
        analysis.append("")
        analysis.append("### 🔍 **Gaussian Noise Robustness**")
        analysis.append("")
        analysis.append("1. **WST shows superior robustness**: Maintains consistent performance ")
        analysis.append("   across all gaussian noise levels, particularly in SUNSET area.")
        analysis.append("")
        ...
        analysis.append("   gaussian noise, especially at higher noise levels (σ=50).")
        analysis.append("")
        
        analysis.append("### 🎯 **Gaussian Noise Mechanisms**")
        analysis.append("")
        analysis.append("**WST**: Multi-scale coefficients capture structural patterns that are ")
        analysis.append("less affected by additive gaussian noise. The wavelet transform's ")
        ...
        analysis.append("the additive nature of gaussian noise, which affects local pixel ")
        analysis.append("intensity distributions and statistical measures.")
        analysis.append("")
        
        analysis.append("### 📊 **Practical Recommendations**")
        analysis.append("")
        analysis.append("1. **For gaussian-critical applications**: Use **WST**, especially in ")
    ...
    def create_comparison_plots(self, output_dir: str):
        """Create comparison plots for gaussian noise experiments."""
        if not self.results:
            self.load_all_experiments()
        ...
        plt.ylim(0.5, 1.05)
        plt.tight_layout()
        plt.savefig(comp_dir / "accuracy_vs_gaussian_noise_overall.png", dpi=300, bbox_inches='tight')
        plt.close()
        ...
        plt.title('Accuracy vs Dataset Size\n(Averaged over geographic areas, gaussian noise conditions, and k values)', fontsize=14)
        ...
        plt.title('Accuracy Distribution by Method - Gaussian Noise Experiments\n(All experiments)', fontsize=14)
        plt.xlabel('Feature Extraction Method', fontsize=12)
        plt.ylabel('Accuracy', fontsize=12)
        plt.grid(True, alpha=0.3)
        plt.ylim(0.5, 1.05)
        plt.tight_layout()
        plt.savefig(comp_dir / "accuracy_vs_method_boxplot_gaussian.png", dpi=300, bbox_inches='tight')
        plt.close()
        ...
        plt.title('Accuracy Heatmap: Methods vs Gaussian Noise-Dataset Combinations\n(Averaged over geographic areas and k values)', fontsize=12)
        plt.xlabel('Condition (Noise + Dataset)', fontsize=11)
        plt.ylabel('Feature Extraction Method', fontsize=11)
        plt.xticks(rotation=45)
        plt.tight_layout()
        plt.savefig(comp_dir / "accuracy_heatmap_gaussian_summary.png", dpi=300, bbox_inches='tight')
        plt.close()
        
        print(f"✅ Gaussian noise comparison plots saved to: {comp_dir}/")
    ...
    def create_detailed_plots(self, output_dir: str):
        ...
        print("Creating accuracy vs gaussian noise plots...")
        for dataset_type in dataset_order:
            for k_val in k_order:
                ...
                if len(subset_data) > 0:
                    ...
                    plt.title(f'Accuracy vs Gaussian Noise - {dataset_label} Dataset, k={k_val}\n(Averaged over geographic areas only)', fontsize=12)
                    ...
                    plt.ylim(0.5, 1.05)
                    plt.tight_layout()
                    plt.savefig(detail_dir / f"accuracy_vs_gaussian_{dataset_type}_k{k_val}.png", dpi=300, bbox_inches='tight')
                    plot_count += 1
                plt.close()
        ...
        print(f"✅ Detailed gaussian noise plots saved to: {detail_dir}/")
    ...
    def export_to_csv(self, output_dir: str):
        ...
        csv_path = Path(output_dir) / "gaussian_experiments_summary.csv"
        export_df.to_csv(csv_path, index=False)
        print(f"📊 Gaussian noise results exported to: {csv_path}")
    
    def create_complete_analysis(self, output_dir: str = "gaussian_analysis"):
        """Create complete analysis with all plots and reports for gaussian noise experiments."""
        output_path = Path(output_dir)
        output_path.mkdir(exist_ok=True)
        
        print("🚀 Starting complete gaussian noise analysis...")
        ...
        with open(output_path / "gaussian_comprehensive_report.md", "w", encoding="utf-8") as f:
            f.write(comprehensive_report)
        
        print("📝 Generating qualitative analysis...")
        qualitative_analysis = self.generate_qualitative_analysis()
        with open(output_path / "gaussian_qualitative_analysis.md", "w", encoding="utf-8") as f:
        ...
        print(f"\n✅ Complete gaussian noise analysis finished!")
    ...
    def create_analysis_summary(self, output_dir: Path):
        ...
        summary = []
        summary.append("# GAUSSIAN NOISE ANALYSIS SUMMARY")
        summary.append("=" * 50)
        summary.append("")
        ...
        summary.append("- `gaussian_comprehensive_report.md`: Complete statistical analysis")
        summary.append("- `gaussian_qualitative_analysis.md`: In-depth qualitative interpretation")
        summary.append("- `gaussian_experiments_summary.csv`: Raw data export")
        summary.append("")
        ...
        if not df.empty:
            ...
            # Noise impact
            if 'clean' in df['noise_condition'].values and 'gaussian50' in df['noise_condition'].values:
                clean_acc = df[df['noise_condition'] == 'clean']['mean_accuracy'].mean()
                gaussian50_acc = df[df['noise_condition'] == 'gaussian50']['mean_accuracy'].mean()
                noise_impact = (clean_acc - gaussian50_acc) * 100
                summary.append(f"- **Gaussian noise impact**: {noise_impact:.1f}% performance loss (Clean → Gaussian σ=50)")
        ...
        with open(output_dir / "gaussian_analysis_summary.md", "w", encoding="utf-8") as f:
            f.write("\n".join(summary))
        
        print(f"📋 Gaussian analysis summary saved to: {output_dir / 'gaussian_analysis_summary.md'}")

def main():
    """Main function to run complete gaussian noise analysis."""
    analyzer = GaussianExperimentAnalyzer()
