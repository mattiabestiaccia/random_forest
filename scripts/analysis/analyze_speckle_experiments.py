#!/usr/bin/env python3
"""
Complete analysis script for speckle noise experiments.
...
class SpeckleExperimentAnalyzer:
    def __init__(self, base_dir: str = "/home/brusc/Projects/random_forest/experiments_organized/speckle_output"):
        self.base_dir = Path(base_dir)
        self.results = []
        self.noise_conditions = ['clean', 'speckle15', 'speckle35', 'speckle55']
        ...
        self.noise_labels = {
            'clean': 'Clean', 
            'speckle15': 'Speckle ν=0.15', 
            'speckle35': 'Speckle ν=0.35', 
            'speckle55': 'Speckle ν=0.55'
        }
    ...
    def load_all_experiments(self) -> List[Dict]:
        """Load all experiment files from speckle_output directories."""
        experiments = []
        
        # Look for directories matching the speckle pattern
        for noise_level in ['15', '35', '55']:
            noise_dir = self.base_dir / f"dataset_rgb_speckle_{noise_level}"
                        ...
                        for method in self.feature_methods:
                            ...
                            for json_file in json_files:
                                try:
                                    ...
                                    exp_data = {
                                        'noise_condition': f'speckle{noise_level}',
                                        'area': area,
    ...
    def generate_comprehensive_report(self) -> str:
        """Generate a comprehensive summary report for speckle noise experiments."""
        if not self.results:
        ...
        report = []
        report.append("# COMPARATIVE REPORT: RANDOM FOREST EXPERIMENTS - SPECKLE NOISE")
        report.append("=" * 70)
        report.append("")
        ...
        report.append("under different speckle noise conditions and dataset sizes.")
        report.append("")
        ...
        report.append("## SPECKLE NOISE ROBUSTNESS ANALYSIS")
        ...
        for exp in self.results:
            ...
            elif exp['noise_condition'] == 'speckle15':
                speckle15_perfs[key] = exp['mean_accuracy']
            elif exp['noise_condition'] == 'speckle35':
                speckle35_perfs[key] = exp['mean_accuracy']
            elif exp['noise_condition'] == 'speckle55':
                speckle55_perfs[key] = exp['mean_accuracy']
        
        # Calculate degradation by method
        method_degradation_15 = {}
        method_degradation_35 = {}
        method_degradation_55 = {}
        
        for key in clean_perfs:
            area, dataset, method, k = key
            if method not in method_degradation_15:
                method_degradation_15[method] = []
                method_degradation_35[method] = []
                method_degradation_55[method] = []
            
            if key in speckle15_perfs:
                degradation = clean_perfs[key] - speckle15_perfs[key]
                method_degradation_15[method].append(degradation)
            
            if key in speckle35_perfs:
                degradation = clean_perfs[key] - speckle35_perfs[key]
                method_degradation_35[method].append(degradation)
                
            if key in speckle55_perfs:
                degradation = clean_perfs[key] - speckle55_perfs[key]
                method_degradation_55[method].append(degradation)
        
        report.append("### Average degradation per method (Clean → Speckle ν=0.15)")
        ...
        report.append("")
        report.append("### Average degradation per method (Clean → Speckle ν=0.35)")
        for method in sorted(method_degradation_35.keys()):
            if method_degradation_35[method]:
                mean_deg = statistics.mean(method_degradation_35[method])
                std_deg = statistics.stdev(method_degradation_35[method]) if len(method_degradation_35[method]) > 1 else 0
                method_name = self.method_labels.get(method, method)
                report.append(f"- {method_name}: {mean_deg:.3f} ± {std_deg:.3f}")
        
        report.append("")
        report.append("### Average degradation per method (Clean → Speckle ν=0.55)")
        for method in sorted(method_degradation_55.keys()):
            if method_degradation_55[method]:
                mean_deg = statistics.mean(method_degradation_55[method])
                std_deg = statistics.stdev(method_degradation_55[method]) if len(method_degradation_55[method]) > 1 else 0
                method_name = self.method_labels.get(method, method)
                report.append(f"- {method_name}: {mean_deg:.3f} ± {std_deg:.3f}")
    ...
    def generate_qualitative_analysis(self) -> str:
        """Generate qualitative analysis for speckle noise robustness."""
        if not self.results:
            self.load_all_experiments()
        ...
        analysis = []
        analysis.append("# QUALITATIVE ANALYSIS: WST vs RGB STATISTICS ROBUSTNESS TO SPECKLE NOISE")
        analysis.append("")
        analysis.append("## EXECUTIVE SUMMARY")
        analysis.append("")
        analysis.append("Comparative analysis reveals significant differences in speckle noise ")
        ...
        analysis.append("superior performance compared to **Advanced RGB Statistics** under speckle noise conditions.")
        analysis.append("")
        ...
        analysis.append("### 2. SPECKLE NOISE ROBUSTNESS")
        analysis.append("")
        ...
        analysis.append("")
        analysis.append("### 🔍 **Speckle Noise Robustness**")
        analysis.append("")
        analysis.append("1. **WST shows superior robustness**: Maintains consistent performance ")
        analysis.append("   across all speckle noise levels, particularly in SUNSET area.")
        analysis.append("")
        ...
        analysis.append("   speckle noise, especially at higher noise levels (ν=0.55).")
        analysis.append("")
        
        analysis.append("### 🎯 **Speckle Noise Mechanisms**")
        analysis.append("")
        analysis.append("**WST**: Multi-scale coefficients capture structural patterns that are ")
        analysis.append("less affected by multiplicative speckle noise. The wavelet transform's ")
        ...
        analysis.append("the multiplicative nature of speckle noise, which affects local pixel ")
        analysis.append("intensity distributions.")
        analysis.append("")
        
        analysis.append("### 📊 **Practical Recommendations**")
        analysis.append("")
        analysis.append("1. **For speckle-critical applications**: Use **WST**, especially in ")
        analysis.append("   environments with speckle noise ν > 0.35.")
        analysis.append("")
    ...
    def create_comparison_plots(self, output_dir: str):
        """Create comparison plots for speckle noise experiments."""
        if not self.results:
            self.load_all_experiments()
        ...
        noise_order = ['clean', 'speckle15', 'speckle35', 'speckle55']
        ...
        plt.ylim(0.5, 1.05)
        plt.tight_layout()
        plt.savefig(comp_dir / "accuracy_vs_speckle_noise_overall.png", dpi=300, bbox_inches='tight')
        plt.close()
        ...
        plt.title('Accuracy vs Dataset Size\n(Averaged over geographic areas, speckle noise conditions, and k values)', fontsize=14)
        ...
        plt.title('Accuracy Distribution by Method - Speckle Noise Experiments\n(All experiments)', fontsize=14)
        plt.xlabel('Feature Extraction Method', fontsize=12)
        plt.ylabel('Accuracy', fontsize=12)
        plt.grid(True, alpha=0.3)
        plt.ylim(0.5, 1.05)
        plt.tight_layout()
        plt.savefig(comp_dir / "accuracy_vs_method_boxplot_speckle.png", dpi=300, bbox_inches='tight')
        plt.close()
        ...
        plt.title('Accuracy Heatmap: Methods vs Speckle Noise-Dataset Combinations\n(Averaged over geographic areas and k values)', fontsize=12)
        plt.xlabel('Condition (Noise + Dataset)', fontsize=11)
        plt.ylabel('Feature Extraction Method', fontsize=11)
        plt.xticks(rotation=45)
        plt.tight_layout()
        plt.savefig(comp_dir / "accuracy_heatmap_speckle_summary.png", dpi=300, bbox_inches='tight')
        plt.close()
        
        print(f"✅ Speckle noise comparison plots saved to: {comp_dir}/")
    ...
    def create_detailed_plots(self, output_dir: str):
        ...
        print("Creating accuracy vs speckle noise plots...")
        for dataset_type in dataset_order:
            for k_val in k_order:
                ...
                if len(subset_data) > 0:
                    ...
                    plt.title(f'Accuracy vs Speckle Noise - {dataset_label} Dataset, k={k_val}\n(Averaged over geographic areas only)', fontsize=12)
                    ...
                    plt.ylim(0.5, 1.05)
                    plt.tight_layout()
                    plt.savefig(detail_dir / f"accuracy_vs_speckle_{dataset_type}_k{k_val}.png", dpi=300, bbox_inches='tight')
                    plot_count += 1
                plt.close()
        ...
        print(f"✅ Detailed speckle noise plots saved to: {detail_dir}/")
    ...
    def export_to_csv(self, output_dir: str):
        ...
        csv_path = Path(output_dir) / "speckle_experiments_summary.csv"
        export_df.to_csv(csv_path, index=False)
        print(f"📊 Speckle noise results exported to: {csv_path}")
    
    def create_complete_analysis(self, output_dir: str = "speckle_analysis"):
        """Create complete analysis with all plots and reports for speckle noise experiments."""
        output_path = Path(output_dir)
        output_path.mkdir(exist_ok=True)
        
        print("🚀 Starting complete speckle noise analysis...")
        ...
        with open(output_path / "speckle_comprehensive_report.md", "w", encoding="utf-8") as f:
            f.write(comprehensive_report)
        
        print("📝 Generating qualitative analysis...")
        qualitative_analysis = self.generate_qualitative_analysis()
        with open(output_path / "speckle_qualitative_analysis.md", "w", encoding="utf-8") as f:
        ...
        print(f"\n✅ Complete speckle noise analysis finished!")
    ...
    def create_analysis_summary(self, output_dir: Path):
        ...
        summary = []
        summary.append("# SPECKLE NOISE ANALYSIS SUMMARY")
        summary.append("=" * 50)
        summary.append("")
        ...
        summary.append("- `speckle_comprehensive_report.md`: Complete statistical analysis")
        summary.append("- `speckle_qualitative_analysis.md`: In-depth qualitative interpretation")
        summary.append("- `speckle_experiments_summary.csv`: Raw data export")
        summary.append("")
        ...
        if not df.empty:
            ...
            # Noise impact
            if 'clean' in df['noise_condition'].values and 'speckle55' in df['noise_condition'].values:
                clean_acc = df[df['noise_condition'] == 'clean']['mean_accuracy'].mean()
                speckle55_acc = df[df['noise_condition'] == 'speckle55']['mean_accuracy'].mean()
                noise_impact = (clean_acc - speckle55_acc) * 100
                summary.append(f"- **Speckle noise impact**: {noise_impact:.1f}% performance loss (Clean → Speckle ν=0.55)")
        
        summary.append("")
        summary.append("## METHODOLOGY")
        summary.append("")
        summary.append("- **Geographic areas**: Results averaged over assatigue, popolar, sunset")
        summary.append("- **Noise conditions**: clean, speckle15 (ν=0.15), speckle35 (ν=0.35), speckle55 (ν=0.55)")
        ...
        with open(output_dir / "speckle_analysis_summary.md", "w", encoding="utf-8") as f:
            f.write("\n".join(summary))
        
        print(f"📋 Speckle analysis summary saved to: {output_dir / 'speckle_analysis_summary.md'}")

def main():
    """Main function to run complete speckle noise analysis."""
    analyzer = SpeckleExperimentAnalyzer()
