#!/usr/bin/env python3
"""
Complete analysis script for salt and pepper noise experiments.
...
class SaltPepperExperimentAnalyzer:
    def __init__(self, base_dir: str = "/home/brusc/Projects/random_forest/experiments/s&p"):
        self.base_dir = Path(base_dir)
        self.results = []
        self.noise_conditions = ['clean', 'saltpepper5', 'saltpepper15', 'saltpepper25']
    ...
    def load_all_experiments(self) -> List[Dict]:
        """Load all experiment files from salt_and_pepper_output directories."""
        experiments = []
        
        # Look for directories matching the salt and pepper pattern
        for noise_level in ['5', '15', '25']:
            noise_dir = self.base_dir / f"dataset_rgb_salt_and_pepper_{noise_level}"
                ...
                for dataset in self.datasets:
                    ...
                    for k in self.k_values:
                        ...
                        for method in self.feature_methods:
                            ...
                            # Search for experiment report JSON files
                            json_files = list(method_dir.glob("experiment_report_with_model.json"))
                            for json_file in json_files:
                                try:
                                    ...
                                    exp_data = {
                                        'noise_condition': f'saltpepper{noise_level}',
                                        'area': area,
    ...
    def generate_comprehensive_report(self) -> str:
        """Generate a comprehensive summary report for salt and pepper noise experiments."""
        if not self.results:
        ...
        report = []
        report.append("# COMPARATIVE REPORT: RANDOM FOREST EXPERIMENTS - SALT & PEPPER NOISE")
        report.append("=" * 70)
        report.append("")
        ...
        report.append("under different salt and pepper noise conditions and dataset sizes.")
        report.append("")
        ...
        report.append("## SALT & PEPPER NOISE ROBUSTNESS ANALYSIS")
        ...
        for exp in self.results:
            ...
            elif exp['noise_condition'] == 'saltpepper5':
                sp5_perfs[key] = exp['mean_accuracy']
            elif exp['noise_condition'] == 'saltpepper15':
                sp15_perfs[key] = exp['mean_accuracy']
            elif exp['noise_condition'] == 'saltpepper25':
                sp25_perfs[key] = exp['mean_accuracy']
        
        # Calculate degradation by method
        method_degradation_5 = {}
        method_degradation_15 = {}
        method_degradation_25 = {}
        
        for key in clean_perfs:
            area, dataset, method, k = key
            if method not in method_degradation_5:
                method_degradation_5[method] = []
                method_degradation_15[method] = []
                method_degradation_25[method] = []
            
            if key in sp5_perfs:
                degradation = clean_perfs[key] - sp5_perfs[key]
                method_degradation_5[method].append(degradation)
            
            if key in sp15_perfs:
                degradation = clean_perfs[key] - sp15_perfs[key]
                method_degradation_15[method].append(degradation)
                
            if key in sp25_perfs:
                degradation = clean_perfs[key] - sp25_perfs[key]
                method_degradation_25[method].append(degradation)
        
        report.append("### Average degradation per method (Clean → S&P 5%)")
        for method in sorted(method_degradation_5.keys()):
            if method_degradation_5[method]:
                mean_deg = statistics.mean(method_degradation_5[method])
                std_deg = statistics.stdev(method_degradation_5[method]) if len(method_degradation_5[method]) > 1 else 0
                method_name = self.method_labels.get(method, method)
                report.append(f"- {method_name}: {mean_deg:.3f} ± {std_deg:.3f}")
        
        report.append("")
        report.append("### Average degradation per method (Clean → S&P 15%)")
        for method in sorted(method_degradation_15.keys()):
            if method_degradation_15[method]:
                mean_deg = statistics.mean(method_degradation_15[method])
                std_deg = statistics.stdev(method_degradation_15[method]) if len(method_degradation_15[method]) > 1 else 0
                method_name = self.method_labels.get(method, method)
                report.append(f"- {method_name}: {mean_deg:.3f} ± {std_deg:.3f}")

        report.append("")
        report.append("### Average degradation per method (Clean → S&P 25%)")
    ...
    def generate_qualitative_analysis(self) -> str:
        """Generate qualitative analysis for salt and pepper noise robustness."""
        if not self.results:
            self.load_all_experiments()
        ...
        analysis = []
        analysis.append("# QUALITATIVE ANALYSIS: WST vs RGB STATISTICS ROBUSTNESS TO SALT & PEPPER NOISE")
        analysis.append("")
        analysis.append("## EXECUTIVE SUMMARY")
        analysis.append("")
        analysis.append("Comparative analysis reveals significant differences in salt & pepper noise ")
        ...
        analysis.append("superior performance compared to **Advanced RGB Statistics** under salt & pepper noise conditions.")
        analysis.append("")
        ...
        analysis.append("### 2. SALT & PEPPER NOISE ROBUSTNESS")
        analysis.append("")
        ...
        analysis.append("")
        analysis.append("### 🔍 **Salt & Pepper Noise Robustness**")
        analysis.append("")
        analysis.append("1. **WST shows superior robustness**: Maintains consistent performance ")
        analysis.append("   across all salt & pepper noise levels, particularly in SUNSET area.")
        analysis.append("")
        ...
        analysis.append("   salt & pepper noise, especially at higher noise levels (25%).")
        analysis.append("")
        
        analysis.append("### 🎯 **Salt & Pepper Noise Mechanisms**")
        analysis.append("")
        analysis.append("**WST**: Multi-scale coefficients capture structural patterns that are ")
        analysis.append("less affected by impulse noise characteristics of salt & pepper noise. The wavelet ")
        analysis.append("transform's multi-resolution analysis provides inherent noise resilience.")
        analysis.append("")
        analysis.append("**Advanced RGB Stats**: Single-channel statistics are more sensitive to ")
        analysis.append("the extreme pixel values introduced by salt & pepper noise, which ")
        analysis.append("significantly distorts mean, variance, and higher-order moment calculations.")
        analysis.append("")
        
        analysis.append("### 📊 **Practical Recommendations**")
        analysis.append("")
        analysis.append("1. **For impulse/transmission noise applications**: Use **WST**, especially in ")
        analysis.append("   environments with salt & pepper noise > 15%.")
        analysis.append("")
    ...
    def create_comparison_plots(self, output_dir: str):
        """Create comparison plots for salt and pepper noise experiments."""
        if not self.results:
            self.load_all_experiments()
        ...
        noise_order = ['clean', 'saltpepper5', 'saltpepper15', 'saltpepper25']
        ...
        plt.ylim(0.5, 1.05)
        plt.tight_layout()
        plt.savefig(comp_dir / "accuracy_vs_saltpepper_noise_overall.png", dpi=300, bbox_inches='tight')
        plt.close()
        ...
        plt.title('Accuracy vs Dataset Size\n(Averaged over geographic areas, salt & pepper noise conditions, and k values)', fontsize=14)
        ...
        plt.title('Accuracy Distribution by Method - Salt & Pepper Noise Experiments\n(All experiments)', fontsize=14)
        plt.xlabel('Feature Extraction Method', fontsize=12)
        plt.ylabel('Accuracy', fontsize=12)
        plt.grid(True, alpha=0.3)
        plt.ylim(0.5, 1.05)
        plt.tight_layout()
        plt.savefig(comp_dir / "accuracy_vs_method_boxplot_saltpepper.png", dpi=300, bbox_inches='tight')
        plt.close()
        ...
        plt.title('Accuracy Heatmap: Methods vs Salt & Pepper Noise-Dataset Combinations\n(Averaged over geographic areas and k values)', fontsize=12)
        plt.xlabel('Condition (Noise + Dataset)', fontsize=11)
        plt.ylabel('Feature Extraction Method', fontsize=11)
        plt.xticks(rotation=45)
        plt.tight_layout()
        plt.savefig(comp_dir / "accuracy_heatmap_saltpepper_summary.png", dpi=300, bbox_inches='tight')
        plt.close()
        
        print(f"✅ Salt & pepper noise comparison plots saved to: {comp_dir}/")
    ...
    def create_detailed_plots(self, output_dir: str):
        ...
        print("Creating accuracy vs salt & pepper noise plots...")
        for dataset_type in dataset_order:
            for k_val in k_order:
                ...
                if len(subset_data) > 0:
                    for method in methods:
                        ...
                        plot_noise_errors = [err if not pd.isna(noise_stats[i]) else 0 for i, err in enumerate(noise_errors)]
                    ...
                    plt.title(f'Accuracy vs Salt & Pepper Noise - {dataset_label} Dataset, k={k_val}\n(Averaged over geographic areas only)', fontsize=12)
                    ...
                    plt.ylim(0.5, 1.05)
                    plt.tight_layout()
                    plt.savefig(detail_dir / f"accuracy_vs_saltpepper_{dataset_type}_k{k_val}.png", dpi=300, bbox_inches='tight')
                    plot_count += 1
                plt.close()
        ...
        print(f"✅ Detailed salt & pepper noise plots saved to: {detail_dir}/")
    ...
    def export_to_csv(self, output_dir: str):
        ...
        csv_path = Path(output_dir) / "saltpepper_experiments_summary.csv"
        export_df.to_csv(csv_path, index=False)
        print(f"📊 Salt & pepper noise results exported to: {csv_path}")
    
    def create_complete_analysis(self, output_dir: str = "saltpepper_analysis"):
        """Create complete analysis with all plots and reports for salt and pepper noise experiments."""
        output_path = Path(output_dir)
        output_path.mkdir(exist_ok=True)
        
        print("🚀 Starting complete salt & pepper noise analysis...")
        ...
        with open(output_path / "saltpepper_comprehensive_report.md", "w", encoding="utf-8") as f:
            f.write(comprehensive_report)
        
        print("📝 Generating qualitative analysis...")
        qualitative_analysis = self.generate_qualitative_analysis()
        with open(output_path / "saltpepper_qualitative_analysis.md", "w", encoding="utf-8") as f:
        ...
        print(f"\n✅ Complete salt & pepper noise analysis finished!")
    ...
    def create_analysis_summary(self, output_dir: Path):
        ...
        summary = []
        summary.append("# SALT & PEPPER NOISE ANALYSIS SUMMARY")
        summary.append("=" * 50)
        summary.append("")
        ...
        summary.append("- `saltpepper_comprehensive_report.md`: Complete statistical analysis")
        summary.append("- `saltpepper_qualitative_analysis.md`: In-depth qualitative interpretation")
        summary.append("- `saltpepper_experiments_summary.csv`: Raw data export")
        summary.append("")
        ...
        if not df.empty:
            ...
            # Noise impact
            if 'clean' in df['noise_condition'].values and 'saltpepper25' in df['noise_condition'].values:
                clean_acc = df[df['noise_condition'] == 'clean']['mean_accuracy'].mean()
                sp25_acc = df[df['noise_condition'] == 'saltpepper25']['mean_accuracy'].mean()
                noise_impact = (clean_acc - sp25_acc) * 100
                summary.append(f"- **Salt & pepper noise impact**: {noise_impact:.1f}% performance loss (Clean → S&P 25%)")
        
        summary.append("")
        summary.append("## METHODOLOGY")
        summary.append("")
        summary.append("- **Geographic areas**: Results averaged over assatigue, popolar, sunset")
        summary.append("- **Noise conditions**: clean, saltpepper5 (5%), saltpepper15 (15%), saltpepper25 (25%)")
        ...
        with open(output_dir / "saltpepper_analysis_summary.md", "w", encoding="utf-8") as f:
            f.write("\n".join(summary))
        
        print(f"📋 Salt & pepper analysis summary saved to: {output_dir / 'saltpepper_analysis_summary.md'}")

def main():
    """Main function to run complete salt and pepper noise analysis."""
    analyzer = SaltPepperExperimentAnalyzer()
